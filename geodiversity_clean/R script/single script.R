...
# Top 15 nodes
top_nodes <- head(node_weights$Node, 15)
...
mat <- matrix(
  0,
  nrow = length(top_nodes),
  ncol = length(top_nodes),
  dimnames = list(top_nodes, top_nodes)
)

for (i in seq_len(nrow(edges_top))) {
  src <- edges_top$Source[i]
  tgt <- edges_top$Target[i]
  w   <- edges_top$Weight[i]

  mat[src, tgt] <- mat[src, tgt] + w
  mat[tgt, src] <- mat[tgt, src] + w
}

# --- Pastel brown–orange gradient ---
palette <- colorRampPalette(c(
  "#f3e5d8",  # very light beige
  "#e6c3a1",
  "#d9a066",
  "#c97a3a",
  "#b25a1b"
...
# --- Add labels perpendicular (90°) to arcs ---
circos.trackPlotRegion(
  ...
  panel.fun = function(x, y) {
    ...
    circos.text(
      ...
      y = ylim[1] + 0.1,
      labels = sector.name,
      facing = "clockwise",   # perpendicular to arc
      niceFacing = TRUE,      # auto-flips for readability
      adj = c(0, 0.5),
      cex = 0.8
    )
  },
